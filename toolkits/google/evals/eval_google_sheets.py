...
def create_spreadsheet_eval() -> EvalSuite:
    ...
    sheet_content_sparse_expected = """{"1": {"AA": "=SUM(SEQUENCE(100))", "3782": {"A": 3783, "D": 3784, "AAZ": 3785, "ZZFS": 3786, "CA": 3787}}}"""
    ...
    suite.add_case(
        ...
        user_message="Create a spreadsheet named 'Sparse Data' that fills the 27th column in the first row with the formula that generates the numbers 1 through 100 and then sums them. The 3782nd row should have its A, D, AAZ, ZZFS, and CA columns filled with the numbers 1, 2, 3, 4, and 5, respectively, summed with its row number.",
        expected_tool_calls=[
            ExpectedToolCall(
                func=create_spreadsheet,
                args={
