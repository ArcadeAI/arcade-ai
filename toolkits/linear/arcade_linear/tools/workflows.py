...
async def get_workflow_states(
    context: ToolContext,
    team: Annotated[
        str, "Team name or ID to get workflow states for (e.g. 'Frontend', 'Backend', 'Product')."
    ],
...
async def create_workflow_state(
    context: ToolContext,
    team: Annotated[
        str, "Team name or ID to create the workflow state in (e.g. 'Frontend', 'Backend')."
    ],
    name: Annotated[
    ...
    **Tips:**
    ...
    try:
        ...
        if "duplicate" in error_msg or "already exists" in error_msg:
            ...
            return {"error": f"Failed to create workflow state '{name}': {e!s}"}
