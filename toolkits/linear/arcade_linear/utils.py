"""Linear utilities for resolving and formatting data"""
...
def normalize_priority(priority: str | int | None) -> int | None:
    """Normalize priority input to integer value for Linear API"""
    if priority is None:
        return None
...
def _build_basic_filters(
    ...
    """Helper to build basic single-value filters"""
    if team_id:
...
def _build_date_filters(
    ...
    completed_at_gte: datetime | None,
    completed_at_lte: datetime | None,
    due_date_gte: datetime | None,
    due_date_lte: datetime | None,
) -> None:
...
def _build_search_filters(filter_obj: dict[str, Any], search_query: str) -> None:
    """Helper to build search filters from query"""
    # Split search query into keywords and filter common words
    ...
    if len(keywords) == 0:
        # No meaningful keywords - use original search
        filter_obj["or"] = [
        ...
        # Multiple keywords - simplified approach
        all_conditions = []
        for kw in keywords:
            all_conditions.extend([
                {"title": {"containsIgnoreCase": kw}},
                {"description": {"containsIgnoreCase": kw}},
                {"labels": {"some": {"name": {"containsIgnoreCase": kw}}}},
            ])
...
def build_issue_filter(
    ...
    # Build basic filters
    _build_basic_filters(
        filter_obj, team_id, assignee_id, creator_id, state_id, priority, cycle_id, parent_id
    )
    ...
    # Handle text search
    if search_query:
        _build_search_filters(filter_obj, search_query)
...
async def resolve_issue_identifier_to_uuid(
    ...
    try:
        ...
        context.logger.debug(f"Issue resolution failed: {e}")
...
async def resolve_team_by_name(context: ToolContext, team_name: str) -> dict[str, Any] | None:
    ...
    exact_matches = [team for team in teams if team["name"].lower() == team_name.lower()]
    ...
    partial_matches = [team for team in teams if team_name.lower() in team["name"].lower()]
...
async def resolve_user_by_email_or_name(
    ...
    exact_matches = [user for user in users if user["name"].lower() == identifier.lower()]
    if len(exact_matches) == 1:
        return exact_matches[0]
    elif len(exact_matches) > 1:
...
async def resolve_workflow_state_by_name(
    ...
    # Look for exact match
    exact_matches = [state for state in states if state["name"].lower() == state_name.lower()]
    if len(exact_matches) == 1:
        return exact_matches[0]
    elif len(exact_matches) > 1:
        state_names = [f"{state['name']} ({state['team']['name']})" for state in exact_matches]
        raise ToolExecutionError(  # noqa: TRY003
    ...
    if len(states) == 1:
        ...
        state_names = [f"{state['name']} ({state['team']['name']})" for state in states]
        raise ToolExecutionError(  # noqa: TRY003
...
async def resolve_project_by_name(context: ToolContext, project_name: str) -> dict[str, Any] | None:
...
async def resolve_projects_by_name(context: ToolContext, project_name: str) -> list[dict[str, Any]]:
    ...
    for project in projects:
        ...
        if normalized_project == normalized_search or project_name_lower == space_to_underscore:
...
def _match_cycle_by_number(cycles: list[dict[str, Any]], cycle_name: str) -> dict[str, Any] | None:
    """Helper to match cycle by number if input is numeric"""
...
def _match_cycle_by_name(cycles: list[dict[str, Any]], cycle_name: str) -> dict[str, Any] | None:
    """Helper to match cycle by exact and partial name"""
...
async def resolve_cycle_by_name(
    ...
    result = _match_cycle_by_number(cycles, cycle_name)
    if result:
        return result

    # Try matching by name
    return _match_cycle_by_name(cycles, cycle_name)


def _parse_cycle_date(cycle: dict[str, Any]) -> datetime | None:
    """Parse cycle start date with timezone handling"""
...
def _get_cycles_with_dates(cycles: list[dict[str, Any]]) -> list[tuple[dict[str, Any], datetime]]:
    """Get cycles with parsed dates and sort by date"""
    cycles_with_dates = [
        (cycle, _parse_cycle_date(cycle))
        for cycle in cycles
        if _parse_cycle_date(cycle) is not None
    ]
    cycles_with_dates.sort(key=lambda x: x[1])  # Sort by date
    return cycles_with_dates


async def _handle_next_cycle(
    client: LinearClient, team_id: str, cycles_with_dates: list[tuple[dict[str, Any], datetime]]
) -> dict[str, Any] | None:
    """Handle 'next' cycle reference"""
    from datetime import datetime, timezone
    ...
    if current_cycle:
        current_starts = _parse_cycle_date(current_cycle)
        if current_starts:
...
def _handle_previous_cycle(
    cycles_with_dates: list[tuple[dict[str, Any], datetime]],
) -> dict[str, Any] | None:
    """Handle 'previous' or 'last' cycle reference"""
    from datetime import datetime, timezone
...
def _parse_date_based_reference(cycle_ref_lower: str) -> datetime | None:
    """Parse date-based cycle references into target dates"""
    from datetime import datetime, timedelta, timezone
    ...
    in_pattern = re.match(r"in\s+(\d+)\s+(week|weeks|day|days|month|months)", cycle_ref_lower)
    if in_pattern:
...
def _find_cycle_containing_date(
    cycles_with_dates: list[tuple[dict[str, Any], datetime]], target_date: datetime
) -> dict[str, Any] | None:
    """Find cycle that contains the target date"""
    from datetime import datetime, timezone
...
async def resolve_cycle_by_relative_reference(
    ...
    """Resolve cycles by relative references like 'current', 'next', 'previous',
    or date-based like 'in 3 weeks'
    ...
    cycles_with_dates = _get_cycles_with_dates(cycles)

    # Handle 'next' - find next cycle after current
    if cycle_ref_lower == "next":
        return await _handle_next_cycle(client, team_id, cycles_with_dates)

    # Handle 'previous' or 'last' - find most recently completed cycle
    if cycle_ref_lower in ["previous", "last"]:
        return _handle_previous_cycle(cycles_with_dates)

    # Handle date-based references like "in 3 weeks", "in 2 days", "next week"
    target_date = _parse_date_based_reference(cycle_ref_lower)

    # If we have a target date, find the cycle that contains it
    if target_date:
        return _find_cycle_containing_date(cycles_with_dates, target_date)
...
async def resolve_labels_by_names(
    ...
    for label_name in label_names:
        ...
        # Look for exact match
        exact_matches = [label for label in labels if label["name"].lower() == label_name.lower()]
        if len(exact_matches) == 1:
        ...
        if label_name_lower in labels_by_name:
            ...
            if team_label:
                ...
                await _create_team_specific_label(
                    context, client, label_name, team_id, resolved_labels, label_name_lower
                )
        else:
...
async def _create_team_specific_label(
    ...
    try:
        ...
        if create_result.get("success") and create_result.get("label"):
            ...
            _raise_label_creation_error(label_name)

    except Exception as e:
        error_msg = str(e)
        if "Duplicate label" in error_msg or "already exists" in error_msg:
            # The label might already exist but wasn't returned by the API
            # Try to fetch it again
            try:
                existing_labels = await resolve_labels_read_only(context, [label_name], team_id)
                if existing_labels:
                    resolved_labels.append(existing_labels[0])
                    return
                else:
                    # Still can't find it, this shouldn't happen
                    _handle_duplicate_label_error(label_name)
            except Exception:
                # If we can't fetch labels again, provide a helpful error
                _handle_duplicate_label_error(label_name)
        else:
            # Other error, re-raise with original message
            _raise_label_error(label_name, e)


def _raise_label_creation_error(label_name: str) -> None:
    """Helper to raise label creation errors"""
    raise ToolExecutionError("Failed") from None


def _raise_label_error(label_name: str, cause: Exception | None = None) -> None:
    """Helper to raise label errors with optional cause"""
    if cause:
        raise ToolExecutionError("Error") from cause
    else:
        raise ToolExecutionError("Error") from None


def _handle_duplicate_label_error(label_name: str) -> None:
    """Helper function to handle duplicate label errors during retrieval"""
    raise ToolExecutionError("Missing") from None
...
async def resolve_labels_read_only(
    ...
    available_labels_map = {label["name"].lower(): label for label in available_labels}
    ...
    if missing_labels:
        team_info = "in this team" if team_id else "in the workspace"

        if len(missing_labels) == 1:
            error_msg = (
                f"Label '{missing_labels[0]}' not found {team_info}. " "Labels are case-sensitive."
            )
        else:
            error_msg = (
                f"Labels not found {team_info}: {', '.join(missing_labels)}. "
        ...
        if available_labels:
            ...
            if len(available_labels) > 10:
                error_msg += (
                    f" Available labels include: '{available_list}' and "
                    f"{len(available_labels) - 10} others."
                )
            else:
...
def _clean_issue_nested_objects(issue_data: dict[str, Any], cleaned: dict[str, Any]) -> None:
    """Helper to clean nested objects in issue data"""
...
def _clean_issue_collections(issue_data: dict[str, Any], cleaned: dict[str, Any]) -> None:
    """Helper to clean collections in issue data"""
    if issue_data.get("labels") and issue_data["labels"].get("nodes"):
        cleaned["labels"] = [clean_label_data(label) for label in issue_data["labels"]["nodes"]]

    if issue_data.get("children") and issue_data["children"].get("nodes"):
        cleaned["children"] = [clean_issue_data(child) for child in issue_data["children"]["nodes"]]
    ...
    if issue_data.get("relations") and issue_data["relations"].get("nodes"):
        cleaned["relations"] = [
            clean_relation_data(relation) for relation in issue_data["relations"]["nodes"]
        ]
    ...
    if issue_data.get("attachments") and issue_data["attachments"].get("nodes"):
        cleaned["attachments"] = [
            clean_attachment_data(attachment) for attachment in issue_data["attachments"]["nodes"]
        ]
...
def clean_issue_data(issue_data: dict[str, Any]) -> dict[str, Any]:
    ...
    _clean_issue_nested_objects(issue_data, cleaned)
    _clean_issue_collections(issue_data, cleaned)
...
def clean_cycle_data(cycle_data: dict[str, Any]) -> dict[str, Any]:
    ...
    if cycle_data.get("issues") and cycle_data["issues"].get("nodes"):
        cleaned["issues"] = [clean_issue_data(issue) for issue in cycle_data["issues"]["nodes"]]
